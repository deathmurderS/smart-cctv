const EventTable = ({ events }) => {
    return (
        <div style={{
            ...
            boxShadow: '0 4px 20px rgba(247,197,208,0.3)', border: '1px solid #f7c5d040'
        }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                <span style={{ fontSize: '20px' }}>⚡</span>
                <h3 style={{ margin: 0, color: '#4a4a4a', fontSize: '15px' }}>Recent Events</h3>
            </div>
            <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '400px' }}>
                    <thead>
                        <tr style={{ background: 'linear-gradient(135deg, #fde8ed, #ede8f7)' }}>
                            {['Camera', 'Location', 'Event', 'Time'].map(h => (
                            ...
                            <tr key={event.id} style={{ background: i % 2 === 0 ? 'white' : '#fdf6f0' }}>
                                ...
                                <td style={{ padding: '12px 16px', borderBottom: '1px solid #fde8ed' }}>
                                    <span style={{
                                        ...
                                        color: '#c9b8e8', padding: '3px 10px',
                                        borderRadius: '20px', fontSize: '12px', fontWeight: '600'
                                    }}>
                                        {event.type}
                                    </span>
                                </td>
                                <td style={{ padding: '12px 16px', borderBottom: '1px solid #fde8ed', color: '#8a8a8a', fontSize: '13px', whiteSpace: 'nowrap' }}>
